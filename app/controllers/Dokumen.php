...
class Dokumen extends Controller
{
    ...
    public function upload_dokumen()
    {
        ...
        if ($this->model('MahasiswaModel')->create($_POST) > 0) {
            if ($this->model('DokumenModel')->uploadDokumen($_POST, $_FILES) > 0) {
                redirectWithMsg(BASE_URL . '/Home/upload', 'Data mahasiswa dan dokumen berhasil disimpan.', 'success');
                exit;
            }
        } else {
            redirectWithMsg(BASE_URL . '/Home/upload', 'Data mahasiswa gagal disimpan.', 'danger');
            exit;
        }
    }
    ...
    public function status_publish()
    {
        ...
        $resolveRedirect = function ($token) {
            ...
            if (strtolower($token) === 'pengajuan')
                return BASE_URL . '/Pengajuan';
            if (strtolower($token) === 'dashboard')
    ...
    public function status()
    {
        ...
        $resolveRedirect = function ($token) {
            ...
            $token = trim($token, " /\\");
            if (strtolower($token) === 'pengajuan')
                return BASE_URL . '/Dokumen/pengajuan';
            if (strtolower($token) === 'dashboard')
                return BASE_URL . '/Dashboard';
            if (strpos($token, '/') !== false) {
                if (preg_match('/^[a-zA-Z0-9_\/\-]+$/', $token)) {
